...
window.onload = function () {
    ...
    let outStr = "";
    for (let i = 0; i < 8; i++) {
        outStr += '<li><span><img src=' + data[i].data.Img + ' width=135px height=200px></span> </br> <span class="name">' + data[i].data.Name + "</span></li > ";
    }
    ...
    pagez.onclick = function () {
        ...
        outStr = "";
        for (let i = pageNum * 8; i < pageNum * 8 + 8; i++) { //通过页数获取data中相应段的值
            outStr += '<li><span><img src=' + data[i].data.Img + ' width=180px height=280px></span> </br> <span class="name">' + data[i].data.Name + "</span></li > ";
        }
        oOl.innerHTML = outStr;
    }
